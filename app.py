...
def load_historical(ticker: str) -> pd.DataFrame:
    """
    Load 10y history from Parquet if present;
    otherwise fetch via yfinance and save.
    """
    fp = DATA_DIR / f"{ticker}_{HIST_WINDOW}.parquet"
    if fp.exists():
...
# Latest SPY via Stooq + VIX from history
...
vix = vix.assign(
    Spike=lambda d: np.where(d["Close"]>=th3, "3SD",
...
summary = pd.DataFrame({
    ...
    "Value":    [current_spy_price, current_vix_price, th2, th3, f"{vix_pct:.1f}th"],
    "As Of":    [as_of_date]*2 + ["-","-","-"]
})
...
with col1:
    ...
    ax.legend(); ax.set_ylabel("Index (100→)")
    st.pyplot(fig)
